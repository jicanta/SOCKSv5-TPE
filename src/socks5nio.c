...
static void socks5_destroy_(struct socks5 *s) {
    if (s->origin_resolution) freeaddrinfo(s->origin_resolution);
...
static void socks5_destroy(struct socks5 *s) {
    if (!s) return;
    if (s->references == 1) {
        if (s->origin_resolution) { freeaddrinfo(s->origin_resolution); s->origin_resolution = NULL; }
        if (s->username) { free(s->username); s->username = NULL; }
        
        if (pool_size < max_pool) {
            s->next = pool; pool = s; pool_size++;
        } else {
            socks5_destroy_(s);
        }
    } else {
...
static void done_arrival(const unsigned state, struct selector_key *key) { (void)state; (void)key; }
static void error_arrival(const unsigned state, struct selector_key *key) { (void)state; (void)key; }

static const struct state_definition client_states[] = {
    { .state = HELLO_READ, .on_arrival = hello_read_init, .on_read_ready = hello_read },
    { .state = HELLO_WRITE, .on_write_ready = hello_write },
    { .state = AUTH_READ, .on_arrival = auth_read_init, .on_read_ready = auth_read },
    { .state = AUTH_WRITE, .on_write_ready = auth_write },
    { .state = REQUEST_READ, .on_arrival = request_read_init, .on_read_ready = request_read },
    ...
    { .state = COPY, .on_arrival = copy_init, .on_read_ready = copy_read, .on_write_ready = copy_write },
...
static void socksv5_done(struct selector_key *key) {
    ...
    if (s->client_fd >= 0) { selector_unregister_fd(key->s, s->client_fd); close(s->client_fd); s->client_fd = -1; }
    if (s->origin_fd >= 0) { selector_unregister_fd(key->s, s->origin_fd); close(s->origin_fd); s->origin_fd = -1; }
    active_connections--;
}
...
static void socksv5_read(struct selector_key *key) {
    ...
    if (st == DONE || st == ERROR) socksv5_done(key);
}
...
void socksv5_passive_accept(struct selector_key *key) {
    ...
    if (s == NULL) { close(client_fd); return; }
    ...
    if (selector_register(key->s, client_fd, &socks5_handler, OP_READ, s) != SELECTOR_SUCCESS) {
        socks5_destroy(s); close(client_fd); return;
    }
    active_connections++;
